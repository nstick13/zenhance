...
- **V2 canvas was the live map** until the orbital map replaced it (2026-09-13) and it was **retired entirely on 2026-09-24** — see [V2.md](V2.md) for the full build log and [ENGINES.md](ENGINES.md) for how to read the code back. Shipped through V2.2: persisted positions, analytics overlays, scenario mode, drag-to-reassign, team reparenting, inline create/edit/delete for people and teams, and (v0.1.16–22) the cross-cutting seat model.
...
- **`/org` is the orbital map, and only the orbital map** (2026-09-24, v0.1.55). Canvas and radial are gone; `?view=canvas` and `?view=radial` fall through to orbital rather than 404.
...
- **~~Retire the radial~~ — done 2026-09-24**, but *without* the port this entry asked for. `RadialOrg.tsx` owned the **findings rail** and the **formal (`managerId`) layer**, and both went with it. The maths survives untouched (`lib/analytics/findings.ts`, `allocation.ts`, `gaps.ts`, `rollup.ts` — listed in [ENGINES.md](ENGINES.md) § Currently unreferenced); only the UI is gone. **Rebuilding the findings rail on the orbital map is now unscheduled work that used to be shipped** — it belongs in the analytics design pass, which is design-first. The formal layer has no UI at all now.
