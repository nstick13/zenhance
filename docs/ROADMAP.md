...
This is **role-coverage analytics on `disciplineId`** — which S4 already half-does ("Tideway has no QA"). ✅ **SHIPPED v0.1.39 as S5 tab 4 (Standards).** The drawn design (`design/PodTemplate.dc.html`) built as specced: role *ranges* rather than fixed counts, a sortable gap table rather than the map (*"they're not going to want blobs"*), answerable on customer data now that S2 import fills `disciplineId`. See the S5 section for detail.
...
**Map default lens now edits from Settings (v0.1.37).** The S5 **Map defaults** tab (tab 3) edits the workspace-default colour-by/label-by directly, via the same `saveLens` the topbar's "Make default" calls — so the org-wide default finally has a home that isn't "flip your own view and promote it." No new `Lens` fields, no migration, guardrail intact.

**Outstanding — the further toggles** (cross-cutting render mode, shared-people rail as an option, stat lines, stream headers). *Unblocked since v0.1.35; each still needs its render code added alongside the toggle (that's the guardrail — a toggle only exists once something reads it), so they land with their canvas work, not as bare settings rows.*
...
### S5 — Workspace settings (the config surface)  *(all five tabs shipped)* ✅
**Shipped:** `/settings` (`app/(app)/settings/page.tsx` + `components/settings/SettingsManager.tsx`) with **Vocabulary**, **Disciplines**, **Map defaults** (v0.1.37), **Findings** (v0.1.38, layers 1–2), and **Standards** / pod template (v0.1.39). The only S5 work left is the deferred exceptions layer of tab 5 (below).

> **⚠️ Two migrations to apply in Neon before deploy — `0006_brainy_zarda.sql` (`findings_policy`) and `0007_last_nightmare.sql` (`pod_template_roles`).** Same hand-apply rule as `0003`/`0004`/`0005`: paste the bare SQL into Neon's SQL editor **before** the code deploys. Both are *new tables*, not `ALTER workspaces`, so the blast radius is narrow — `requireWorkspace` touches neither, and both readers (`getFindingsPolicy`, `getPodTemplate`/`getPodGaps`) swallow the "relation does not exist" case and fall back (default policy / empty template), so **/org and /settings will not 500 even before the tables exist.** Writes from the two tabs fail until applied, and the demo seed's starter pod template is skipped (guarded) until then. `0006` includes a partial unique index (one workspace-wide config row) that Drizzle's schema can't express and that lives in the `.sql` by hand — **apply the whole file, not a regenerated diff.**
...
3. ✅ **Map defaults** (v0.1.37) — edits the workspace-default lens (colour-by/label-by) via `saveLens`. S3's further toggles still land with their render code.
4. ✅ **Standards** (v0.1.39) — the pod template: role *ranges* (min–max per discipline) in the new `pod_template_roles` table, and a compare-to-ideal **gap table** rendered right in the tab (Team · Composition · Gap, sortable, worst-first) — never on the canvas, per "they're not going to want blobs". Pure model in `lib/analytics/podTemplate.ts`; the demo org seeds a starter template (Engineering 2–6, QA 1+) that reproduces the planted "Tideway has no QA" gap. This answers the CTO's "how do our 90 pods rate" question on data S2 import made real.
5. ✅ **Findings** (v0.1.38, layers 1–2) — detector on/off + per-discipline Spread threshold, in the new `findings_policy` table. Wired into `computeAllFindings` (the v0.1.36 split), so a discipline set to "no limit" simply never fires Spread — this is also how the old hard-coded "platform is exempt" is now expressed. **Layer 3 — per-instance exceptions** (file a finding as reviewed with a reason; the "9 of 12 reviewed" counter) — **deliberately deferred:** it needs finding-instance identity and a review UI that only pays off once findings render on the *live canvas*, which they don't yet (findings still live only in the unlinked radial). Sequencing note: **show findings on the canvas before building the exceptions layer.**
...
**① SHIPPED v0.1.36 — the detector splits in two, both count-based.** `computeOverAllocFindings` is gone; `lib/analytics/findings.ts` now exposes **`computeSpreadFindings`** (per-person team count ≥ threshold) and **`computeOverCommitmentFindings`** (declared > 100%). The two are independent, so Devraj (4t/120%) fires under both while Grace (2t/80%) now fires only as Spread — never mislabelled "over-allocated." Kinds/colours: `spread` (indigo `#8b9df0`), `over-commitment` (the old red `#ff7a8a`), `coupling` (amber). 20 Vitest cases. **Nate's two reserved calls, now decided:** Spread threshold is **2** (`SPREAD_TEAM_THRESHOLD`, a seeded workspace-wide default — `computeSpreadFindings` takes it as an argument so per-discipline policy can override it later); and the *no % in findings* rule holds — **Spread carries a team count, only Over-commitment carries the %**. ⚠️ At threshold 2, Spread mirrors the cross-team population (539 at Heather's scale), so the per-discipline policy table (below) is what makes it tractable — it is the follow-on this split was built to feed.

**① (design) "On 2+ teams" is a FACT. Whether it becomes a finding is a per-discipline rule the *organisation* sets.**
...
**③ Findings config = detector on/off → per-discipline policy → exceptions.** Exceptions are filed per instance with a reason, attributed and dated; the "9 of 12 reviewed" counter is itself the signal. **This is the first tab needing real tables rather than a jsonb blob** — the actual justification for the settings surface. **Layers 1–2 SHIPPED v0.1.38 (S5 tab 5); layer 3 (exceptions) deferred — see below.**
...
**~~Still open~~ Resolved v0.1.36:** the "declared %" stays — but only on **Over-commitment**, where the number *is* the load. Spread is %-free, so the *no % in findings* rule holds for the structural finding while the load finding keeps the one number that means something.
...
- **Value-stream-level roles (train roles)** *(discuss — Data Model + Analytics)* — roles that belong to the **value stream / train itself**, not to a team inside it: RTE / release-train engineer, value-stream architect, product management, delivery lead, business owner. Distinct from two neighbouring ideas and needs to be kept distinct from both: from **discipline/role on a team** (the story above, which attaches to a `team` unit), and from **cross-cutting / shared people** (Analytics, below — a person who *splits across* teams). A train role is a named seat on the *group* rung.
  ...
  - **Ideal value-stream template (compare-to-ideal at the group rung)** — the direct twin of the shipped **Standards / pod template** (S5 tab 4: ideal role *ranges* per team in `pod_template_roles` + a worst-first gap table via `lib/analytics/podTemplate.ts`). A value stream should have a defined ideal cabinet — every stream needs a business leader, an RTE / program coordinator, an architect, etc. — and the same **compare-to-ideal gap table** ("Stream · Cabinet · Gap", sortable, worst-first) answers *"which of our streams is missing a leader / RTE?"* on real data. Model it as the group-rung parallel of the pod template (a `stream_template_roles` shape, or generalise `pod_template_roles` across both `kind`s) and reuse the pod-template analytics rather than re-deriving. Lives in `/settings` alongside Standards.
  - Payoff: ownership analytics (streams with no RTE / no architect), the product cabinet per stream, and a natural place for the AI import to drop a "this person looks like your RTE" guess. Ties into **Ownership analytics** (backlog), **Standards / pod template** (S5 tab 4, the team-rung precedent), and the **AI-assisted import** feature.
