...
- **Version 0.1.34** on `main`, **not yet deployed** — the `0005` migration below must be applied in Neon first. Last production deploy was 0.1.33. Stack: Vercel + Neon Postgres + Clerk auth. Migrations `0003` (people attributes) and `0004` (`workspaces.lens`) must be applied by hand in Neon's SQL editor **before** the deploy — Drizzle's `select()` names every column, so an unapplied migration 500s every query touching that table, and `0004`'s table is `workspaces`, which means the whole app.
- 🚨 **Migration `0005_stormy_red_ghost.sql` (`workspaces.vocabulary`) must be pasted into Neon's SQL editor BEFORE the next deploy.** Same hazard as `0004`, same table: `requireWorkspace` does `select({ workspace: workspaces })`, so until the column exists **every page 500s**. Applied locally already. The exact statement:
  ```sql
  ALTER TABLE "workspaces" ADD COLUMN "vocabulary" jsonb;
  ```
  Like `0001`/`0003`/`0004` it is **not** in Drizzle's `__drizzle_migrations` ledger, so a future `db:migrate` may try to replay it.
