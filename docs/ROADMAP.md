...
- **Version 0.1.33**, deployed to **production** on Vercel (Neon Postgres + Clerk auth). Migrations `0003` (people attributes) and `0004` (`workspaces.lens`) must be applied by hand in Neon's SQL editor **before** the deploy — Drizzle's `select()` names every column, so an unapplied migration 500s every query touching that table, and `0004`'s table is `workspaces`, which means the whole app.
