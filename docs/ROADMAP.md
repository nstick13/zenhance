...
### S5 — Workspace settings (the config surface)  *(tabs 1–3 + 5 shipped; tab 4 next)*
**Shipped:** `/settings` (`app/(app)/settings/page.tsx` + `components/settings/SettingsManager.tsx`) with **Vocabulary**, **Disciplines**, **Map defaults** (v0.1.37), and **Findings** (v0.1.38, layers 1–2). Tab 4 (Standards / pod template) is the only one left, plus the deferred exceptions layer of tab 5.

> **⚠️ Migration to apply before deploy — `0006_brainy_zarda.sql` (`findings_policy` table).** Same hand-apply rule as `0003`/`0004`/`0005`: paste the bare SQL into Neon's SQL editor **before** the code deploys. Unlike those, this is a *new table*, not an `ALTER workspaces`, so the blast radius is narrower — `requireWorkspace` doesn't touch it, and `getFindingsPolicy` swallows the "relation does not exist" case and falls back to the default policy, so /org will not 500 even in the window before the table exists. The Findings tab's writes will fail until it's applied, though. The migration includes a partial unique index (one workspace-wide config row) that Drizzle's schema can't express — it's in the `.sql` by hand, so **apply the whole file, not a regenerated diff.**
...
4. **Standards** — the pod template (ideal role composition). Feeds compare-to-ideal. *Only tab left.*
