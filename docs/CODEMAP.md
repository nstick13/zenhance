...
| **Blank-canvas start** (grow an org one node at a time; the team appears rather than being declared) | `app/lab/grow/GrowLab.tsx` — shared SVG feel-study engine with zoom/pan and in-memory creation; `visualRules.ts` owns the study's ring reveal, colours and person spacing. Thin `page.tsx` route. `app/lab/grow-established/fixture.ts` supplies an invented 45-person fixture; `/grow-established` aliases the lab route for review. No DB. Integration decisions and unfinished seams: [UNIFIED-ORBITAL.md](UNIFIED-ORBITAL.md); study history: [LAB.md](LAB.md) § `grow` |
...
| **Demo companies** | `npm run db:companies` (`lib/db/companies.ts`) seeded **Sparrow Jam Manufacturing, OH** (10 people) and **Northwind Freight & Logistics** (~2,500 over 12 rungs). As of 2026-09-19, Northwind is retained as a scale fixture and design record but hidden from the interactive dev-account switcher (`lib/auth/workspace.ts`); the large import card and interactive seed action are disabled while Greg settles the visuals using Sparrow and the 45-person Digital Tailoring demo. Existing customer workspaces are untouched. |
