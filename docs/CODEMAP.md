...
| **Hovering things that aren't Konva nodes** | Work items, progress rings and the torus are *painted*, not noded — several hundred dots would be several hundred nodes. `hitTest` in `OrbitalMap.tsx` finds them arithmetically from the same geometry the painters use (`ringGeometry` is shared by both, deliberately). Clicking a work item opens the right-hand panel and from there `PersonTaskBoard` |
