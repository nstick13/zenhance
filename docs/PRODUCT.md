...
- **2026-09-13** — **Drag on the orbital map rearranges the *view*, not the org.** `orbital_nodes.parentId` is an override: dropping a team into another stream persists the arrangement without touching `org_units.parent_id`. Promoting an arrangement to a real restructure is a separate, deliberate act — **not yet built**. *(Greg's design; Nate agreed 2026-09-13.)*
- **2026-09-13** — ⚠️ **Open: one gesture, three meanings.** Orbital drag = view-only; Canvas person→team = real edit (staged when Scenario mode is on); Canvas team→stream = real edit, always immediate, never staged. Nothing on screen says which you're in, and **Scenario mode is unreachable from the default view** (`OrbitalMap.tsx` has no `moves`/`performMove`). Also orphans the "Scenario save / compare" roadmap item, which builds on the in-memory `moves` overlay. **Next step: a visible "arrangement only — org unchanged" affordance on the orbital map, plus a "Make this real" action.** Decide the label after UAT.
- **2026-09-13** — **The orbital map is the default `/org` view**; canvas → `?view=canvas`, radial → `?view=radial`. Rationale recorded only in a code comment at the time (Greg: *"this needs to replace the current view as the default visualisation and behaviour"*); logged here retroactively.
- **2026-09-13** — **Multi-company switching shipped**, retiring "v1 is one-user-per-workspace". The switcher's choice rides in a cookie that is **never trusted as a capability**: it only selects among workspaces the user already belongs to, validated on write (`selectWorkspace`) and re-validated on read (`getOrCreateWorkspace`). Prerequisite for the PM/consultant ICP, where one user carries N clients.
- **2026-09-13** — 🧭 **Open positioning question: PM/consultant/scrum-master as the ICP breaks the free/paid seam above.** That seam assumes Configurer (free) recruits Consumer (paid) — but a consultant *is both*, so there's no second persona to recruit and gating "executive insight" gates the reason they came. If this ICP is real, the fence likely moves to **multi-client + output** (N workspaces, the shareable artifact handed to the client, scenario compare, branding) and insight becomes the demo. **Not decided.** Also unsettles the Enterprise tier (integration build projects), which is the motion this ICP deprioritises.
